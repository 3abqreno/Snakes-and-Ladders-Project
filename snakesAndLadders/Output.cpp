...
void Output::DrawCardNumber(const CellPosition& cellPos, int cardNum) const
{
	...
	///TODO: Calculate the Width & Height of the integer "cardNum" if written using the Current Font(done)
	...
	///TODO: Draw the Integer the representing the "cardNum" in the location (x,y)(done)
...
void Output::DrawCell(const CellPosition& cellPos, int cardNum) const
{
	...
	///TODO: Get the Width and Height of the Cell Number if written using the current font (done)
	//       (Use GetIntegerSize() window function) and set the "w" and "h" variables with its width and height
	...
	///TODO: Draw the cell number in the x and y location(done)
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if (fromCell.GetCellNum()==1)//check ladder doesn't start at cellnum 1
	{
		return;
	}
	...
	for (int i = 0; i < fromCell.VCell() - toCell.VCell(); i++) {
		...
		pWind->DrawLine(x12, yh, x34, yh);
		yh -= UI.CellHeight;

	}

	///TODO: Draw the cross horizontal lines of the ladder using the appropriate coordinates(done)
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if ((toCell.VCell() <= fromCell.VCell()) || (toCell.HCell() != fromCell.HCell()))//to validate from cell is above to cell
	{
		return;
	}
	if (fromCell.GetCellNum() == 99)//snake head can't be at cellnum 99
		return;
	...
	xp1 = x12 - xChange;
	xp2 = x12 + xChange;
	int xp[] = { x12,xp2,x12,xp1};
	int yp[] = {yp1,y1,yp2,y1 };
