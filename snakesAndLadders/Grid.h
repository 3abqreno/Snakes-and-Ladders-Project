...
public:
	...
	void DeleteAllGameObject();																	  // and  Draws it in the new cell
	...
	bool SetClipboard(Card * card);  // A setter to be used in copy/cut (in order NOT to break class responsibilities)
	...
	bool isOverLappingGrid(GameObject* thisObj)const;
	Player* getNextPlayer(Player* thisPlayer)const;
	Player* GetleastPlayerWallet();
	int GetSnakeNum()const;
	int GetCardNum()const;
	void SaveAll(ofstream& OutFile, ObjectType)const;
	// ========= Other Getters =========
