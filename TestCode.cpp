...
int main()
{
	...
	pOut->DrawPlayer(player_15, 0, UI.PlayerColors[0]);/////invlaid
	pOut->DrawPlayer(player_99, 1, UI.PlayerColors[1]);
	pOut->DrawPlayer(player_99, 5, UI.PlayerColors[1]);//////invlaid
	pOut->DrawPlayer(player_99, -1, UI.PlayerColors[1]);////invalid
	...
	pOut->PrintMessage("you entered: " + to_string(f));
	...
	/// 3.2- Getting Cell Clicked ///(not done)
	/// =========================
	...
	CellPosition cl;
	for (int i = 0; i < 5; i++) {
		cl = pIn->GetCellClicked();
		pOut->PrintMessage("Vcell = " + to_string(cl.VCell()) + " Hcell= " + to_string(cl.HCell()));
	...
	for (int i = 0; i < 5; i++)
	{
	pOut->PrintMessage("please enter Vcell");
	w = pIn->GetInteger(pOut);
	cellpos_1.SetVCell(w);
	pOut->PrintMessage("vcell = " + to_string(cellpos_1.VCell()));
	pIn->GetPointClicked(x, y);
}

	for (int i = 0; i < 5; i++)
	{
		pOut->PrintMessage("please enter Hcell");
		w = pIn->GetInteger(pOut);
		cellpos_1.SetHCell(w);
		pOut->PrintMessage("Hcell = " + to_string(cellpos_1.HCell()));
		pIn->GetPointClicked(x, y);
	}
	...
	/// TODO:(done)
	int a, b;//a is vcel and b is hcell
	for (int i = 0; i < 5; i++)//prints -1 if invalid cellnum

	{
		pOut->PrintMessage("please enter Vcell");
		a = pIn->GetInteger(pOut);
		pOut->PrintMessage("please enter Hcell");
		b = pIn->GetInteger(pOut);
		CellPosition test(a,b);
		pOut->PrintMessage("CellNum= " + to_string(test.GetCellNum()));
		pIn->GetPointClicked(x,y);
	...
	for (int i = 0; i < 5; i++)
	{
		pOut->PrintMessage("please enter valid cell num");
		cn=pIn->GetInteger(pOut);//now cn has user entered cellnum
		CellPosition test1(cn);//now test1 has positin of cellnum user has entered
		pOut->PrintMessage("Vcell = " + to_string(test1.VCell()) + " Hcell = " + to_string(test1.HCell()));
		pIn->GetPointClicked(x,y);
	...
	do
	{
		...
		case TO_PLAY_MODE:
			...
			pOut->PrintPlayersInfo("P0(100,0), P1(100,), P2(100,0), P3(100,0) | Curr=0");
			break;
